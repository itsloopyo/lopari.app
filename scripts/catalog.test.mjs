...
const countBegin = "<!-- BEGIN MOD COUNT -->";
const countEnd = "<!-- END MOD COUNT -->";
...
const readme = `Handwritten introduction with ${countBegin}0${countEnd} mods\n${recentBegin}\nold list\n${recentEnd}\nHandwritten middle\n${begin}\nold table\n${end}\nHandwritten footer\n`;
const wobbly = {
...
test("README lists verified stores without inferring support from detection", (t) => {
  ...
  assert.ok(output.startsWith(`Handwritten introduction with ${countBegin}6${countEnd} mods\n${recentBegin}\n`));
  ...
  assert.ok(output.includes("| Wobbly Life | Beta | Steam, Xbox | [GitHub](https://github.com/itsloopyo/wobbly-life-headtracking) | dev build (2026-09-01) |"));
  assert.ok(output.includes("| External mod | Released | Steam, Xbox | [GitHub](https://github.com/itsloopyo/wobbly-life-headtracking) | v1.2.3 (2026-09-02) |"));
  assert.ok(output.includes("| Steam only | Beta | Steam |"));
  assert.ok(output.includes("| Store labels | Beta | GOG, Epic, Microsoft Store, EA app, Ubisoft Connect, Retail |"));
